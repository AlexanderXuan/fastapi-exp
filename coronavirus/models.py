...
Date: 2023-05-13 14:59:01
LastEditors: AlexXuan xuanxiaoguang@gmail.com
LastEditTime: 2023-05-13 17:16:04
FilePath: /fastapi-exp/coronavirus/models.py
'''
from sqlalchemy import Column, Integer, String, DateTime, BigInteger, ForeignKey, Date, func
from sqlalchemy.orm import relationship

from .database import Base

class City(Base):
    __tablename__ = 'city'  # 数据表表名
    ...
    # __mapper_args__ = {"order_by": country_code}  # 默认是正序，倒序使用country_code.desc()方法
...
class Data(Base):
    __tablename__ = 'data'  # 数据表表名
    id = Column(Integer, primary_key=True, index=True, autoincrement=True)
    city_id = Column(Integer, ForeignKey('city.id'), nullable=False, comment='所属省/直辖市')  # ForeignKey里的字符串格式不是类名.属性名，而是表名.字段名
